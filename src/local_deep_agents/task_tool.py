...
from local_deep_agents.prompts import TASK_DESCRIPTION_PREFIX
...
def _create_task_tool(tools, subagents: list[SubAgent], model, state_schema):
  ...
  for _agent in subagents:
      if "tools" in _agent:
          ...
          _tools = tools
      # create react sub-agent for each of them
  ...
  # 1. Create new messages to pass to subagent - should just be description
  # 2. Call sub-agent
  # 3. Update state with both the subagents response AND any changes to file system (main agent todo use it)

  @tool(description=TASK_DESCRIPTION_PREFIX.format(sub_agents=sub_agents_string))
  def task(
    description: str,
    subagent_type: str, # sub-agent name
    ...
    tool_call_id: Annotated[str, InjectedToolCallId],
      ...
      # isolation is here because we replace it!!!
      ...
      # This one will update (append) parent messages with tool message (sub-agent result)
      return Command(
          update={
